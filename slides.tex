...
\visible<3->{{\fbox{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = user-specified, \textbf{\textcolor{DarkGreen}{mission}} = user-specified}}}

\item[]

\item[Mosh:] Remote terminal app
...
\item Observe number of bytes received every $\tau$

\item[]

\item Update probability distribution of $\lambda$
...
\item Receiver makes forecast; piggybacks on acknowledgment
...
\raisebox{0.8 ex}{\includegraphics[width=3 cm]{Google_Logo_Old.PNG}} & & \includegraphics[width=4 cm]{meraki.png} \\
...
\item Capture the entire state of a VP8 decoder

\item ``Frames'' are functions that operate on state

\item Player chooses best achievable path to reach ``\textit{fin}''

\vspace{\baselineskip}

\item \textcolor{DarkBlue}{\bf Model}: VP8 decoder state machine
