...
\begin{frame}
\frametitle{Declarative design}

\Large

\begin{centering}

\fbox{Systems with a \textcolor{DarkBlue}{\bf model} and a \textcolor{DarkGreen}{\bf mission}.}
...
\begin{frame}
\frametitle{Explicitness in systems design}

\large

\noindent \textcolor{DarkBlue}{\bf Model}: explicit statement of assumptions about the problem
...
Explicit design considerations $\rightarrow$ \textbf{\textcolor{Maroon}{freedom to make changes}}
...
\visible<2->{{\fbox{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = flicker noise, \textbf{\textcolor{DarkGreen}{mission}} = max throughput with delay $<$ 0.1 s}}}

\item[]

\item[Remy:] Computer-generated protocol design

\visible<3->{{\fbox{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = user-specified networks \& workloads, \textbf{\textcolor{DarkGreen}{mission}} = user-specified}}}

\item[]

\item[Mosh:] Mobile shell \& state sync protocol

\visible<4->{{\fbox{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = ANSI terminal emulator, \textbf{\textcolor{DarkGreen}{mission}} = convey latest frame}}}

\item[]

\item[Alfalfa:] Video streaming app

\visible<5->{{\fbox{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = VP8 decoder, \textbf{\textcolor{DarkGreen}{mission}} = quality vs.~startup vs.~stalls}}}
...
\begin{frame}
\frametitle{Verizon LTE ping delay during one TCP download}
...
\raisebox{0.8 ex}{\includegraphics[width=3 cm]{Google_Logo_Old.PNG}} & & \raisebox{-0.3cm}{\includegraphics[width=3 cm]{viber.png}} \\
...
\item What if cell link \emph{isn't} the bottleneck?
...
%KW and Hari Balakrishnan, \textbf{End-to-End Transmission Control by Modeling Uncertainty about the Network State}, \textit{HotNets 2011}
...
\begin{itemize}
\item losing $r\_ewma_\alpha$ hurts the most
\end{itemize}
...
\begin{quote}
\color{DarkBlue} \textit{From the perspective of an endpoint, {\bf what does it help to know} about the network?}
