...
\begin{itemize}
\item network may do \visible<2>{(\textbf{\textcolor{DarkBlue}{model}})}
...
\visible<4->{{\colorbox{Bisque}{\footnotesize\textbf{\textcolor{DarkBlue}{model}} = VP8 decoder, \textbf{\textcolor{DarkGreen}{mission}} = quality vs.~startup vs.~stalls}}}
...
\begin{frame}
\frametitle{Sprout models one user's traffic on one bottleneck}
...
\item What if there are multiple users?
...
\visible<2->{\Large \noindent \hspace{-.5cm} \mbox{\textcolor{DarkBlue}{\textbf{*} Assuming every user is running the same algorithm.}}}
